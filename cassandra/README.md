...
```bash
git clone git@scm.starbucks.com:BITSOPS/cassandra.git
cd cassandra
kubectl create ns cassandra
helm install --name cassandra --namespace cassandra -f values.yaml  --dry-run --debug  .
```
...
The storage class we need to set are as follows

1) managed-standard
2) managed-premium

Run ```kubectl get sc ``` to see all the possible storage classes of the given k8s cluster. 
Note: Not every node support every storage class. 
[More info on Azure VM types] (https://azure.microsoft.com/en-us/pricing/details/virtual-machines/linux/) 

 
Our DB pool nodes support them .
...
For a custom `StorageClass` on other azure, please see documentation here [https://kubernetes.io/docs/user-guide/persistent-volumes/](https://kubernetes.io/docs/user-guide/persistent-volumes/)
...
```bash
helm install --namespace "cassandra" -n "cassandra" --set config.cluster_size=5 .
```
...
For example, you have 6 vms in db pools and you want to deploy cassandra to node which labeled as `db-pool: true`
...
| `persistence.size`                   | Size of data volume                             | `32Gi`                                                     |
...
```bash
helm upgrade --set config.cluster_size=5 cassandra .
```
